...
What distinguishes a token from one another? Why are there so many of them?
...
A program is a much larger bytecode, which requires a huge amount of gas. Typically, deploying several contracts for a tokens costs around a hundreds of dollars.
...
There has been numerous debates on the Bitcoin as the team of developers had diverging opinions that deviated from the original Whitepaper that supposedly set things in stone. As that was opposed to the idea of decentralization, it led to several forks of the Bitcoin Blockchain, with then multiple blockchains running in parallel which essentialy are copies of the Bitcoin, each of them trying to solve issues with the Bitcoin on several sides (transaction speed, reward per block, anonymity, hash function). 
